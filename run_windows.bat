...
cd /d "%~dp0"
title Mavis Digital Campus v0.9.7

echo ===============================================
echo       MAVIS DIGITAL CAMPUS v0.9.7
echo ===============================================
echo.
...
if not "!PORT!"=="8000" echo [INFO] Port 8000 is already in use, probably by an older campus window. v0.9.7 will use !PORT! instead.
echo The browser will open only after THIS version is ready.
echo Keep this window OPEN while you use the campus.
echo.

start "" powershell -NoProfile -WindowStyle Hidden -Command "$url='http://127.0.0.1:!PORT!/?v=098'; for($i=0;$i -lt 80;$i++){ try { $r=Invoke-WebRequest -UseBasicParsing $url -TimeoutSec 1; if($r.StatusCode -ge 200){ Start-Process $url; exit 0 } } catch {}; Start-Sleep -Milliseconds 400 }"
