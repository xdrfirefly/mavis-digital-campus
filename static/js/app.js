...
let contributionFormMode = null;
let peopleSubview = 'directory';
let peopleQuery = '';
let peopleFormKind = null;
let personProfileId = null;
let personEditing = false;
let monthlyRecord = null;
let monthlyRecordLoading = false;
...
function drawerCalendar(){
  ...
  const googleEdit=edit?.source==='google_calendar', google=summary.google_calendar||{};
  ...
  const owned=googleEdit?' readonly aria-readonly="true"':'';
  const locked=googleEdit?' disabled aria-disabled="true"':'';
  const form=`<section class="drawer-section calendar-editor"><div class="section-row"><div><span class="profile-kicker">Campus Calendar</span><h4>${edit?(googleEdit?'Annotate Google event':'Edit event'):'Add event'}</h4></div><span>${googleEdit?'Google-owned schedule':'internal · local'}</span></div><p>${googleEdit?'Google owns the title, schedule, location, and cancellation state. Campus annotations below survive refreshes.':'This Calendar is stored inside Mavis Digital Campus and gives Stella real commitments to plan around.'}</p><form class="memory-form" data-calendar-event-form data-event-id="${edit?.id||''}"><label>Title<input name="title" maxlength="220" required value="${esc(edit?.title||'')}" placeholder="Library class, board meeting, farm workday…"${owned}></label><div class="memory-form-grid"><label>Date<input type="date" name="event_date" required value="${esc(edit?.event_date||summary.today_date||'')}"${owned}></label><label>Type<select name="event_type">${typeOptions}</select></label><label>Starts<input type="time" name="start_time" value="${esc(edit?.start_time||'')}"${owned}></label><label>Ends<input type="time" name="end_time" value="${esc(edit?.end_time||'')}"${owned}></label><label>Commitment<select name="commitment_level">${levelOptions}</select></label><label>Project / Program<select name="project_id"><option value="">None</option>${projectOptions}</select></label><label>Location<input name="location" maxlength="300" value="${esc(edit?.location||'')}" placeholder="Craft Memorial Library"${owned}></label><label>Status<select name="status"${locked}>${statusOptions}</select></label></div><label class="calendar-all-day"><input type="checkbox" name="all_day" value="1"${Number(edit?.all_day||0)?' checked':''}${locked}> All-day event</label><label>Local Campus notes<textarea name="notes" rows="3" maxlength="4000" placeholder="Preparation, travel, what matters…">${esc(edit?.notes||'')}</textarea></label><div class="memory-form-actions"><button class="button-primary" type="submit">${googleEdit?'Save Annotations':edit?'Save Event':'Add Event'}</button>${edit?'<button type="button" class="button-quiet" data-calendar-edit-cancel>Cancel Edit</button>':''}</div></form></section>`;
  ...
  const googleActions=google.connected?`<button type="button" class="button-quiet" data-google-calendar-refresh>Refresh Google Calendar</button><button type="button" class="button-quiet" data-google-calendar-disconnect>Disconnect</button>`:`<button type="button" class="button-quiet" data-google-calendar-connect${google.credentials_configured?'':' disabled'}>Connect Google Calendar</button>`;
  const overview=`<section class="drawer-section calendar-summary"><div class="calendar-summary-grid"><span><strong>${summary.today_count||0}</strong><small>Today</small></span><span><strong>${summary.tomorrow_count||0}</strong><small>Tomorrow</small></span><span><strong>${summary.week_count||0}</strong><small>7 days</small></span></div><p><strong>Stella planning:</strong> Major or multiple same-day commitments automatically reduce the number of extra focus items she recommends.</p><div class="calendar-google-controls"><small>Google Calendar: ${esc(google.connected?'Connected':google.last_refresh_status==='Disconnected'?'Disconnected · cached events retained':summary.external_status||'Not connected')}${google.last_refresh_at?` · last refresh ${esc(fmtTime(google.last_refresh_at,true))}`:''}</small>${googleActions}</div>${!google.credentials_configured?'<small>Add the Google Calendar desktop OAuth client ID to the local .env before connecting.</small>':''}</section>`;
...
function contributionLedgerPanel(){
  ...
  const actions=`<section class="drawer-section"><div class="memory-form-actions"><button class="button-primary" type="button" data-contribution-form="offer">Record Offer</button><button type="button" data-contribution-form="received">Record Received Contribution</button></div></section>`;
  return `<section class="drawer-section contribution-boundary"><strong>Local trusted operator only</strong><span>No authentication or authorization is implemented. These records are excluded from shared state, WebSockets, exports, activity payloads, and AI context.</span></section>${actions}${offerForm}${receivedForm}${offers}${received}`;
}

function peopleTabs(){return `<nav class="people-subnav" aria-label="People views">${[['directory','Directory'],['work','Work Hours'],['contributions','Contributions']].map(([key,label])=>`<button type="button" data-people-view="${key}" class="${peopleSubview===key?'active':''}">${label}</button>`).join('')}</nav>`;}
function personForm(person=null,kind='Person'){
  const edit=!!person,entity=person?.entity_kind||kind,type=person?.person_type||'Volunteer';
  const types=['Volunteer','Staff','Board','Contractor','Collaborator','Sponsor','Community Organization','Other'];
  return `<section class="drawer-section focused-person-form"><div class="section-row"><h4>${edit?'Edit identity':`Add ${esc(kind)}`}</h4><button type="button" data-person-form-close>Close</button></div><form class="memory-form" data-person-form data-person-id="${person?.id||''}"><div class="memory-form-grid"><label>Name<input name="display_name" maxlength="180" required value="${esc(person?.display_name||'')}"></label><label>Identity<select name="entity_kind"><option${entity==='Person'?' selected':''}>Person</option><option${entity==='Organization'?' selected':''}>Organization</option></select></label><label>Role / type<select name="person_type">${types.map(x=>`<option${x===type?' selected':''}>${x}</option>`).join('')}</select></label></div><details><summary>More details</summary><div class="memory-form-grid"><label>Contact information<textarea name="contact_info" rows="2">${esc(person?.contact_info||'')}</textarea></label><label>Status<select name="status"><option${person?.status!=='Inactive'?' selected':''}>Active</option><option${person?.status==='Inactive'?' selected':''}>Inactive</option></select></label></div><label>Operational notes<textarea name="notes" rows="3" maxlength="4000">${esc(person?.notes||'')}</textarea></label><label class="poe-primary-check"><input type="checkbox" name="is_primary_user" value="1"${person?.is_primary_user?' checked':''}> This is me — Person identities only.</label></details><div class="memory-form-actions"><button class="button-primary" type="submit">${edit?'Save Changes':'Add Identity'}</button></div></form></section>`;
}
function manualHoursForm(person){
  const categories=state.activity_categories||[],projects=state.projects||[];
  return `<section class="drawer-section focused-hours-form"><div class="section-row"><h4>Add Hours</h4><button type="button" data-hours-form-close>Close</button></div><form class="memory-form" data-manual-hours-form data-person-id="${person.id}"><div class="memory-form-grid"><label>Date<input type="date" name="work_date" required value="${new Date().toISOString().slice(0,10)}"></label><label>Activity category<select name="activity_category_id" required><option value="">Choose activity…</option>${categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></label><label>Duration (minutes)<input type="number" name="duration_minutes" min="1" step="1" required></label><label>Project / program<select name="project_id"><option value="">No project</option>${projects.map(p=>`<option value="${p.id}">${esc(p.title)}</option>`).join('')}</select></label></div><label>Short description of the work<textarea name="description" rows="2" required maxlength="1000"></textarea></label><details><summary>More details</summary><label>Participation type<select name="participation_type"><option>Volunteer</option><option>Learning</option><option>Paid</option></select></label><label>Additional notes<textarea name="notes" rows="2" maxlength="3000"></textarea></label></details><button class="button-primary" type="submit">Record Hours</button></form></section>`;
}
function participationSummary(person,record=null){
  if((person.entity_kind||'Person')!=='Person')return `<p class="participation-no-target">Organizations do not have a personal monthly participation target.</p>`;
  const minutes=record?record.total_minutes:Number(person.current_month_minutes||0),target=record?record.target_minutes:4800,remaining=Math.max(0,target-minutes);
  return `<div class="participation-summary"><strong>${(minutes/60).toFixed(2).replace(/\.00$/,'')} of ${(target/60).toFixed(0)} hours recorded</strong><span>${remaining?`${(remaining/60).toFixed(2).replace(/\.00$/,'')} hours remaining`:'Monthly participation tracking target reached'}</span><small>Tracking target only; this is not an eligibility determination.</small></div>`;
}
function monthlyParticipationView(person){
  const r=monthlyRecord;
  if(monthlyRecordLoading&&!r)return `${peopleTabs()}<section class="drawer-section"><p>Loading monthly participation record…</p></section>`;
  if(!r)return `${peopleTabs()}<section class="drawer-section"><p>Monthly participation record unavailable.</p></section>`;
  const sessions=r.sessions||[];
  return `${peopleTabs()}<section class="drawer-section monthly-record print-monthly-record"><div class="monthly-record-heading"><span class="profile-kicker">The Mavis Institute</span><h3>Monthly Participation Record</h3><strong>${esc(r.person.display_name)}</strong><span>${esc(r.month_label)}</span></div><div class="month-navigation"><button type="button" data-month-step="-1">Previous</button><input type="month" value="${esc(r.month)}" data-month-picker><button type="button" data-month-step="1">Next</button></div>${participationSummary(r.person,r)}<div class="monthly-actions no-print"><button type="button" class="button-primary" data-print-monthly>Print Monthly Record</button><a class="button-quiet work-report-export" href="/api/people/${r.person.id}/monthly-participation.csv?month=${encodeURIComponent(r.month)}" download>Download CSV</a><button type="button" data-person-profile="${r.person.id}">Back to Profile</button></div><div class="monthly-session-list">${sessions.map(x=>`<article><strong>${esc(x.local_date)} · ${esc(x.activity_name||'Uncategorized')}</strong><span>${esc(x.notes||'No description recorded.')}</span><small>${esc(x.project_title||'No project / program')} · ${esc(x.participation_type||'')} · ${workMinutesLabel(x.duration_minutes)}</small></article>`).join('')||'<p>No completed Work Sessions were recorded for this month.</p>'}</div><p class="monthly-total"><strong>Monthly total:</strong> ${workMinutesLabel(r.total_minutes)}</p><div class="signature-lines"><span>Verifier / signature: ______________________________</span><span>Signature date: __________________</span></div><small>Generated ${esc(r.generated_on)} · ${esc(r.timezone_name)}</small><p class="participation-statement">${esc(r.statement)}</p></section>`;
}
function personProfile(person){
  if(peopleSubview==='monthly')return monthlyParticipationView(person);
  const isPerson=(person.entity_kind||'Person')==='Person',personContributions=(contributionLedger?.contributions||[]).filter(x=>Number(x.contributor_id)===Number(person.id));
  return `${peopleTabs()}<section class="drawer-section person-profile"><span class="profile-kicker">${esc(person.entity_kind||'Person')}</span><h3>${esc(person.display_name)}</h3><p>${esc(person.person_type)} · ${esc(person.status)}</p>${participationSummary(person)}<div class="profile-actions">${isPerson?`<button type="button" class="button-primary" data-add-hours="${person.id}">Add Hours</button><button type="button" data-monthly-record="${person.id}">View Monthly Record</button>${!person.is_primary_user?`<button type="button" data-person-primary="${person.id}">This is me</button>`:''}`:''}<button type="button" data-edit-person="${person.id}">Edit ${isPerson?'Person':'Organization'}</button></div></section>${peopleFormKind==='hours'&&isPerson?manualHoursForm(person):''}${personEditing?personForm(person):''}<section class="drawer-section"><h4>Work Hours</h4><p>${isPerson?'Completed Work Sessions are the authoritative record for participation hours.':'Organizations cannot receive Work Sessions.'}</p></section><section class="drawer-section"><h4>Contributions</h4><p>${personContributions.length} received contribution${personContributions.length===1?'':'s'} linked to this identity.</p><button type="button" data-people-view="contributions">Open Contributions</button></section><section class="drawer-section"><h4>Notes</h4><p>${esc(person.notes||'No operational notes recorded.')}</p></section>`;
}
function drawerPeople(){
  const people=state.people||[],categories=state.activity_categories||[],sessions=state.work_sessions||[],summary=state.work_summary||{},active=sessions.filter(x=>!x.ended_at),recent=sessions.filter(x=>x.ended_at).slice(0,20);
  const selected=people.find(p=>Number(p.id)===Number(personProfileId));if(selected)return personProfile(selected);
  if(peopleSubview==='contributions')return `${peopleTabs()}${contributionLedgerPanel()}`;
  if(peopleSubview==='work'){
    const personOptions=people.filter(p=>p.status==='Active'&&(p.entity_kind||'Person')==='Person').map(p=>`<option value="${p.id}">${esc(p.display_name)} · ${esc(p.person_type)}</option>`).join(''),categoryOptions=categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join(''),projectOptions=(state.projects||[]).map(p=>`<option value="${p.id}">${esc(p.title)}</option>`).join('');
    const clockIn=`<section class="drawer-section"><h4>Clock work in</h4><form class="memory-form" data-clock-in-form><div class="memory-form-grid"><label>Person<select name="person_id" required><option value="">Choose person…</option>${personOptions}</select></label><label>Activity<select name="activity_category_id" required><option value="">Choose activity…</option>${categoryOptions}</select></label><label>Project / Program<select name="project_id"><option value="">No project</option>${projectOptions}</select></label><label>Participation<select name="participation_type"><option>Volunteer</option><option>Learning</option><option>Paid</option></select></label></div><label>What are they doing?<textarea name="notes" rows="2"></textarea></label><button class="button-primary" type="submit">Poe · Clock In</button></form></section>`;
    const activeList=`<section class="drawer-section"><div class="section-row"><h4>Currently Clocked In</h4><span>${active.length}</span></div>${active.map(x=>`<div class="work-session-card active"><div><strong>${esc(x.person_name)}</strong><small>${esc(x.activity_name||'Uncategorized')} · Started ${fmtTime(x.started_at,true)}</small></div><button type="button" data-work-clock-out="${x.id}">Clock Out</button></div>`).join('')||'<p>Nobody is clocked in right now.</p>'}</section>`;
    const recentList=`<section class="drawer-section"><div class="section-row"><h4>Recent Work</h4><span>${recent.length}</span></div>${recent.map(x=>`<div class="work-session-card"><div><strong>${esc(x.person_name)} · ${workMinutesLabel(x.duration_minutes)}</strong><small>${esc(x.activity_name||'Uncategorized')}${x.project_title?` · ${esc(x.project_title)}`:''}</small></div><button type="button" data-work-edit="${x.id}">Correct</button></div>`).join('')||'<p>No completed work sessions yet.</p>'}</section>`;
    return `${peopleTabs()}<section class="drawer-section poe-operations-callout"><div><h4>Work Hours</h4><p>Work Sessions remain the authoritative participation record.</p><button type="button" data-open-panel="work_report">Open Work Hours Report</button></div><div class="work-summary-strip"><span><strong>${summary.active_count||0}</strong> clocked in</span><span><strong>${workMinutesLabel(summary.completed_minutes||0)}</strong> recorded</span></div></section>${poeConversationCard()}${activeList}${clockIn}${recentList}`;
  }
  const q=peopleQuery.trim().toLowerCase(),filtered=people.filter(p=>!q||`${p.display_name} ${p.person_type} ${p.entity_kind} ${p.status}`.toLowerCase().includes(q));
  const form=peopleFormKind==='Person'||peopleFormKind==='Organization'?personForm(null,peopleFormKind):'';
  return `${peopleTabs()}<section class="drawer-section directory-tools"><label>Search directory<input data-people-search value="${esc(peopleQuery)}" placeholder="Name, role, or status"></label><div class="memory-form-actions"><button class="button-primary" type="button" data-add-identity="Person">Add Person</button><button type="button" data-add-identity="Organization">Add Organization</button></div></section>${form}<section class="drawer-section"><div class="section-row"><h4>People & Organizations</h4><span>${filtered.length}</span></div><div class="people-ledger-list">${filtered.map(p=>`<button type="button" class="person-directory-card" data-person-profile="${p.id}"><span><strong>${esc(p.display_name)}</strong><small>${esc(p.entity_kind||'Person')} · ${esc(p.person_type)} · ${esc(p.status)}</small></span><b>${(p.entity_kind||'Person')==='Person'?`${Number(p.current_month_hours||0).toFixed(2).replace(/\.00$/,'')} h this month`:'Organization'}</b></button>`).join('')||'<p>No matching identities.</p>'}</div></section>`;
}
...
async function savePerson(form){
  ...
  const id=Number(form.dataset.personId||0)||null;
  const body={display_name:String(fd.get('display_name')||'').trim(),entity_kind:String(fd.get('entity_kind')||'Person'),person_type:String(fd.get('person_type')||'Volunteer'),status:String(fd.get('status')||'Active'),contact_info:String(fd.get('contact_info')||''),notes:String(fd.get('notes')||''),is_primary_user:fd.get('is_primary_user')==='1',duplicate_acknowledged:false};
  if(!body.display_name){toast('Add a name first.');return;}
  const endpoint=id?`/api/people/${id}`:'/api/people';
  try{let result=await post(endpoint,body);if(result.status==='duplicate_warning'){const names=result.possible_duplicates.map(x=>`${x.display_name} (${x.entity_kind})`).join(', ');if(!confirm(`Possible duplicate: ${names}. Save a separate identity anyway?`))return;result=await post(endpoint,{...body,duplicate_acknowledged:true});}form.reset();peopleFormKind=null;personEditing=false;contributionLedger=null;toast(id?`${body.display_name} updated.`:`${body.display_name} added to Poe’s identity ledger.`);openDrawer('people');}catch(err){toast(err.message);}
}
async function loadMonthlyParticipation(personId,month=monthlyRecordMonth){monthlyRecordLoading=true;monthlyRecord=null;monthlyRecordMonth=month;openDrawer('people');try{const r=await fetch(`/api/people/${personId}/monthly-participation?month=${encodeURIComponent(month)}`,{cache:'no-store'});if(!r.ok)throw new Error(`Monthly record failed (${r.status})`);monthlyRecord=await r.json();monthlyRecordMonth=monthlyRecord.month;}catch(err){toast(err.message);}finally{monthlyRecordLoading=false;openDrawer('people');}}
async function saveManualHours(form){const f=new FormData(form),body={person_id:Number(form.dataset.personId),work_date:String(f.get('work_date')||''),activity_category_id:Number(f.get('activity_category_id')||0),duration_minutes:Number(f.get('duration_minutes')||0),description:String(f.get('description')||'').trim(),project_id:f.get('project_id')?Number(f.get('project_id')):null,participation_type:String(f.get('participation_type')||'Volunteer'),notes:String(f.get('notes')||'').trim()};try{await post('/api/work-sessions/manual',body);peopleFormKind=null;toast('Participation hours recorded.');openDrawer('people');}catch(err){toast(err.message);}}
...
async function connectGoogleCalendar(){try{const result=await post('/api/calendar/google/connect',{});window.open(result.authorization_url,'_blank','noopener');toast('Finish Google authorization in the new browser tab, then return here.');}catch(err){toast(err.message);}}
async function refreshGoogleCalendar(){try{const result=await post('/api/calendar/google/refresh',{});toast(`Google Calendar refreshed · ${result.imported} event(s) in the planning window.`);openDrawer('calendar');}catch(err){toast(err.message);}}
async function disconnectGoogleCalendar(){if(!confirm('Disconnect Google Calendar? Cached imported events will remain in the Campus.'))return;try{await post('/api/calendar/google/disconnect',{});toast('Google Calendar disconnected. Cached events were retained.');openDrawer('calendar');}catch(err){toast(err.message);}}
...
els.drawerBody.addEventListener('click',e=>{
  const btn=e.target.closest('button');if(!btn)return;
  if(btn.dataset.peopleView){peopleSubview=btn.dataset.peopleView;personProfileId=null;peopleFormKind=null;personEditing=false;monthlyRecord=null;openDrawer('people');if(peopleSubview==='contributions')loadContributionLedger();}
  else if(btn.dataset.addIdentity){peopleFormKind=btn.dataset.addIdentity;openDrawer('people');}
  else if(btn.hasAttribute('data-person-form-close')){peopleFormKind=null;personEditing=false;openDrawer('people');}
  else if(btn.dataset.personProfile){personProfileId=Number(btn.dataset.personProfile);peopleSubview='profile';peopleFormKind=null;personEditing=false;monthlyRecord=null;openDrawer('people');}
  else if(btn.dataset.addHours){peopleFormKind='hours';openDrawer('people');}
  else if(btn.hasAttribute('data-hours-form-close')){peopleFormKind=null;openDrawer('people');}
  else if(btn.dataset.editPerson){personEditing=true;openDrawer('people');}
  else if(btn.dataset.monthlyRecord){personProfileId=Number(btn.dataset.monthlyRecord);peopleSubview='monthly';loadMonthlyParticipation(personProfileId,monthlyRecordMonth);}
  else if(btn.dataset.monthStep){const [year,month]=monthlyRecordMonth.split('-').map(Number),d=new Date(Date.UTC(year,month-1+Number(btn.dataset.monthStep),1));loadMonthlyParticipation(personProfileId,d.toISOString().slice(0,7));}
  else if(btn.hasAttribute('data-print-monthly'))window.print();
  else if(btn.dataset.contributionForm){contributionFormMode=btn.dataset.contributionForm;openDrawer('people');}
  else if(btn.hasAttribute('data-contribution-form-close')){contributionFormMode=null;openDrawer('people');}
  else if(btn.dataset.concept)openConcept(btn.dataset.concept);
...
els.drawerBody.addEventListener('input',e=>{
  ...
  if(e.target.matches('[data-people-search]')){peopleQuery=e.target.value;const caret=e.target.selectionStart;openDrawer('people');requestAnimationFrame(()=>{const input=els.drawerBody.querySelector('[data-people-search]');if(input){input.focus({preventScroll:true});try{input.setSelectionRange(caret,caret);}catch{}}});}
});
els.drawerBody.addEventListener('change',e=>{
  if(e.target.matches('[data-month-picker]'))loadMonthlyParticipation(personProfileId,e.target.value);
...
els.drawerBody.addEventListener('submit',e=>{
  ...
  if(form.hasAttribute('data-manual-hours-form')){saveManualHours(form);return;}
